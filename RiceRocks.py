...
class Sprite:
    ...
    def collide(self, rock):
        return dist(self.pos, rock.pos) <= self.radius or dist(self.pos, rock.pos) <= rock.radius
...
def process_sprite_group(rocks, canvas):
    for j in rocks:
        ...
        if j.update():
            rocks = list(rocks)
            rocks.pop(rocks.index(j))
            rocks = set(rocks)
    return rocks
        
def group_collide(obj, rocks):
...
def group_group_collide(missiles, rocks):
    ...
    for missile in missiles:
        if group_collide(missile, rocks):
            score += 1
            return True
    return False
...
def draw(canvas):
    ...
    if lives == 0:
        ...
        lives = 3
        score = 0
    rock_group = process_sprite_group(rock_group, canvas)
...
def rock_spawner():
    ...
    if len(rock_group) >= 7:
        rock_group.pop()
    if dist(rock_pos, my_ship.pos) <= 10:
        return 0
